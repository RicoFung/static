...
$chok.view.query.config.sortPriority = [{"sortName":"id","sortOrder":"asc"}];
...
$chok.view.query.callback.onEditableSave = function(field, row, oldValue, $el){
    ...
    $.ajax({
        ...
        success: function (data, status) {
            if (status=="success") {
        		$.alert({title: "Prompt", content: $chok.checkResult(data)});
    			$("#tb_list").bootstrapTable('refresh'); // 刷新table
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
    		$.alert({title: "Prompt", type: "red", content: jqXHR.status + "<br/>" + jqXHR.responseText});
        }
    });
};
//右键菜单点击事件 
...
$chok.view.query.init.toolbar = function(){
	...
	$("#bar_btn_del").click(function(){
		if($chok.view.query.fn.getIdSelections().length<1) {
			$.alert({title: "Prompt", type: "red", content: "No choice"});
			return;
		}
		$.confirm({
		    title: "Prompt",
		            ...
		            $.ajax({
		                ...
		                success: function(result){
	        	        	...
	        	        	if(result.success==false){
	        	        		$.alert({title: "Prompt", type: "red", content: result.msg});
	        	        		return;
	        	        	}
			    	        $("#tb_list").bootstrapTable('refresh'); // 刷新table
		                },
...
$chok.view.query.fn.getColumns = function(){
	var columns = 
		[
	     ...
	     {title:'operating', field:'operate', align:'center', valign:'middle', width:'50', events:$chok.view.query.config.operateEvents, formatter:$chok.view.query.config.operateFormatter}
		];
